...
# Coords for M shape
xs = [-2, 133, 238, 550, 472, 472, 470, 560, 230, 228, 228, 62, -63, -233, -239, -563, -479, -479, -479, -559, -244,
      -140]
ys = [34, 341, 575, 575, 366, 48, -326, -515, -517, -308, -55, -419, -424, -57, -515, -517, -324, 67, 362, 581, 577,
      349]
...
for t in list(range(N)) + [0]:  # at the end go back to t=0
    ...
    for i in np.flip(
            np.argsort(np.abs(vectors))):  # sort in order of vector length, biggest vectors will be plotted first
        dx = np.real(vectors[i])
        dy = np.imag(vectors[i])
        plt.gcf().gca().add_artist(
            plt.Circle((x, y), np.abs(vectors[i]), color=colors[color_i], alpha=0.05))  # circle, radius is magnitude
